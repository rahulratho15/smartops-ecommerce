...
/* VERSION 3: NEW PRODUCT with broken add-to-cart */
...
document.getElementById('ver-badge').textContent = 'v3 Updated';
...
function add(id) {
    ...
    b.className = 'btn';
    /* V3 BUG: product 9 add-to-cart is not connected to API */
    if (id === 9) {
        slog('CART_BUTTON_BROKEN', { productId: 9, error: 'Add to cart not connected to API for new product' });
        setTimeout(function () { b.textContent = 'Error'; b.className = 'btn fail'; b.disabled = false; }, 1500);
        return;
    }
    fetch(API + '/cart/add', {
        method: 'POST',
