...
document.getElementById('ver-badge').textContent = 'v2 Unstable';
document.getElementById('ver-badge').style.background = '#2a1616';
document.getElementById('ver-badge').style.color = '#f87171';
document.getElementById('ver-badge').style.borderColor = '#3d1f1f';
...
function add(id) {
    ...
    fetch(API + '/cart/add', {
        ...
        .then(function () {
            ...
            /* V2 BUG: trigger animation that crashes the page */
            b.style.transition = 'transform 0.3s';
            b.style.transform = 'scale(1.5) rotate(45deg)';
            setTimeout(function () {
                slog('CRASH_RELOAD', { productId: id, error: 'Animation crash - page reloading' });
                /* Force crash via accessing null property after animation */
                try { document.querySelector('.nonexistent-v2-bug').click(); } catch (e) {
